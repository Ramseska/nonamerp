...
mp.events.add('enableCustomize', () => {
	...
	mp.gui.chat.push(`!{ffff00}[INFO]!{ffffff} Зажмите правую кнопку мыши, чтобы вращать камеру.`);
	...
	tout = setInterval(() => {
		if (mp.keys.isDown(2) === true) {
			if (
				!currentCamPos.x + mp.game.controls.getDisabledControlNormal(0, 220) * sensivityAxis > 403.7 ||
				!currentCamPos.x + mp.game.controls.getDisabledControlNormal(0, 220) * sensivityAxis < 401.7
			) {
				currentCamPos.x = currentCamPos.x + mp.game.controls.getDisabledControlNormal(0, 220) * sensivityAxis;
			}
			if (
				!currentCamPos.z + -mp.game.controls.getDisabledControlNormal(0, 221) * sensivityAxis > -97.5 ||
				!currentCamPos.z + -mp.game.controls.getDisabledControlNormal(0, 221) * sensivityAxis < 99.2
			) {
...
mp.events.add('customizeCameraZoom', (kuda) => {
	switch (kuda) {
		case 0:
			if (creatorCamera.getFov() >= 94) creatorCamera.setFov(94);
			else creatorCamera.setFov(creatorCamera.getFov() + 0.5);
			break;
		case 1:
			if (creatorCamera.getFov() <= 40) creatorCamera.setFov(40);
			else creatorCamera.setFov(creatorCamera.getFov() - 0.5);
			break;
	}
});
...
mp.events.add({
	...
	localCustomizeSetCloth: (type, cloth, toros) => {
		if (parseInt(toros) != -1) lp.setComponentVariation(3, parseInt(toros), 0, 0);
